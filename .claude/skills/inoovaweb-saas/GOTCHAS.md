...
- **A imagem do painel (`ghcr.io/balbiss/saas-chatwoot`) tem o Supabase do dono do kit EMBUTIDO NO BUILD** (`VITE_SUPABASE_URL`/`VITE_SUPABASE_PUBLISHABLE_KEY` são lidas em build-time pelo Vite, e o GitHub Actions do repo sempre usa as Variables do PRÓPRIO repo). Apontar a Application do Coolify pra essa imagem `:latest` faz o painel logar contra o Supabase ERRADO (do dono, não do cliente) — sintoma: "Invalid login credentials" mesmo com email confirmado e senha certa. **Não tem solução só de configuração no Coolify** (env var em runtime não adianta, o valor já foi cravado no bundle JS no build); é preciso buildar uma imagem própria pra cada instalação (`docker build --build-arg VITE_SUPABASE_URL=... --build-arg VITE_SUPABASE_PUBLISHABLE_KEY=... -t ghcr.io/.../saas-chatwoot:<tag-do-cliente> .`, dar push, e apontar a Application pra essa tag) até o painel ser migrado pra ler config em runtime (ex: injetar um `env-config.js` gerado pelo container na subida, em vez de `import.meta.env`).
